...
function realizarBusqueda() {
    ...
    if (terminoBusqueda === '') {
        ...
        peliculasFiltradas = todasLasPeliculas.filter(pelicula =>
            // Busca en el TÍTULO de la película (si existe)
            (pelicula.title && pelicula.title.toLowerCase().includes(terminoBusqueda)) ||
            // O busca en el nombre del DIRECTOR (si existe)
            (pelicula.director && pelicula.director.toLowerCase().includes(terminoBusqueda)) ||
            // O busca en la DESCRIPCIÓN de la película (si existe)
            (pelicula.description && pelicula.description.toLowerCase().includes(terminoBusqueda))
        );
    }
